      ...
      <body>
        <main className=" bg-white relative h-screen max-h-screen overflow-hidden scroll-smooth md:container mx-auto">
          ...
          <div className=" md:container mx-auto relative flex h-full ">
            <div
              className={`md:w-1/4 absolute md:static backdrop-filter backdrop-blur-md bg-opacity-100 h-screen bg-white transition-all duration-300 z-20 ${
                isSidebarOpen ? "left-0 w-64" : "-left-full "
              } md:left-0`}
            >
              <Sidebar otomaticCloseSidebar={handleCloseSidebar} />
            </div>
            <div
              className={`w-screen bg-slate-700 opacity-75 absolute h-screen md:hidden ${
                isSidebarOpen ? "left-0 z-10" : "-left-full "
              } md:left-0`}
